...
def ei_multinom_dirichlet_modified(group_fractions, votes_fractions, precinct_pops, pareto_scale=5, pareto_shape=1):
    """
...
class RowByColumnEI:
    ...
    def fit(
        ...
        check_dimensions_of_input(group_fractions, votes_fractions, precinct_pops, demographic_group_names, candidate_names, self.num_groups_and_num_candidates)
    ...
    def precinct_level_estimates(self):
        ...
        precinct_level_samples = self.sim_trace.get_values("b") # num_samples x num_precincts x r x c
        precinct_posterior_means = precinct_level_samples.mean(axis=0)
        precinct_credible_intervals = np.ones((len(self.precinct_pops), self.num_groups_and_num_candidates[0], self.num_groups_and_num_candidates[1], 2))
        percentiles = [2.5, 97.5]

        for r in range(self.num_groups_and_num_candidates[0]):
            for c in range(self.num_groups_and_num_candidates[1]):
                #precinct_credible_intervals[:, r, c, :] = 
                precinct_credible_intervals[:, r, c, :] = np.percentile(
                    precinct_level_samples[:, :, r, c], percentiles, axis=0
                ).T
    ...
    def plot_intervals(self, group_name, candidate_name):
        if group_name not in self.demographic_group_names:
            raise ValueError(
                'group_name must be a string in the list of demographic_group_names provided to fit()'
            )

        if candidate_name not in self.candidate_names:
            raise ValueError(
                'candidate_name must be a string in the list of candidate_names provided to fit()'
            )
...
def check_dimensions_of_input(group_fractions, votes_fractions, precinct_pops, demographic_group_names, candidate_names, num_groups_and_num_candidates):
    ...
    if demographic_group_names is not None:
        if len(demographic_group_names) != num_groups_and_num_candidates[0]:
            warnings.warn('''Length of demographic_groups_names should be equal to
    ...
    if candidate_names is not None:
        if len(candidate_names) != num_groups_and_num_candidates[1]:
            warnings.warn('''Length of candiate_names should be equal to
    ...
    if len(precinct_pops) != votes_fractions.shape[1]:
        raise ValueError('''votes_fractions should have shape: c x num_precincts.
        In particular, it is required that len(precinct_pops) = votes_fractions.shape[1]
        ''')

    if len(precinct_pops) != group_fractions.shape[1]:
        raise ValueError('''votes_fractions should have shape: r x num_precincts.
