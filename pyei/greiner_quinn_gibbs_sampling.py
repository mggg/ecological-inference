...
def greiner_quinn_gibbs_sample(group_counts, votes_counts, num_samples, nu_0, psi_0, K_0_inv, mu_0, gamma=1):  
    '''
    ...
    if np.all(num_precincts != vote_counts.shape[0]):
        raise ValueError("group_counts and vote_counts must both have first dim of length num_precincts")
    ...
    internal_cell_counts_samp = get_initial_internal_count_sample(group_counts, vote_counts, precinct_pops)
    
    # variables for storing all samples
    internal_cell_counts_samples = np.empty((num_samples, num_precincts, num_groups , num_candidates))
    ...
    for i in range(num_samples):
        ...
        internal_cell_counts_samp = sample_internal_cell_counts(group_counts, votes_counts, theta_samp, internal_cell_counts_samp)       
        internal_cell_counts_samples[i,:,:,:] = internal_cell_counts_samp
        
        # (b) sample theta using Metropolis-Hastings
        theta_samp = sample_theta(internal_cell_counts_samp, theta_samp, omega_samp, mu_samp, Sigma_samp, gamma)
...
def sample_mu(omega, Sigma, K_0_inv, mu_0, num_precincts):
    ...
    mu_n = (np.linalg.inv(K_0_inv + num_precincts * Sigma_inv)) @ (K_0_inv @ mu_0.T + num_precincts * Sigma_inv @ mean_omega)
...
def proposal_dist_generate_sample(mu_samp, Sigma_samp, gamma, num_precincts, deg_freedom=4):
    ...
    '''
    omega_proposed = st.multivariate_t.rvs(mu_samp, gamma * Sigma_samp, df=deg_freedom, size=num_precincts)
...
def log_unnormalized_pdf(theta, omega, internal_cell_counts_samp, mu_samp, Sigma_samp, tol=0.001):
    ...
    if not (np.all( abs(theta.sum(axis=2) - 1) < .01)):  #CHECK IF THETAS SUM TO 1 across rows
        ...
        line_4_and_6 = np.apply_over_axes(np.sum, (internal_cell_counts_samp - 1) * np.log(theta), [1,2]).flatten() #sum over rows and columns
        #line_5 =  - 0.5 * (omega - mu_samp).T @ np.linalg.inv(Sigma_samp) @ (omega - mu_samp) 
        line_5 = - 0.5 * np.linalg.det(Sigma_samp) - 0.5 * ((omega - mu_samp) @ np.linalg.inv(Sigma_samp) * (omega- mu_samp)).sum(axis=1)
...
def sample_theta(internal_cell_counts_samp, theta_prev, omega_prev, mu_samp, Sigma_samp, gamma):
    ...
    omega_proposed = proposal_dist_generate_sample(mu_samp, Sigma_samp, gamma, num_precincts, deg_freedom=4)

    theta_proposed = omega_to_theta(omega_proposed, r, c)

    log_unnormalized_pdf_proposed =  log_unnormalized_pdf(theta_proposed, omega_proposed, internal_cell_counts_samp, mu_samp, Sigma_samp)
    log_unnormalized_pdf_prev = log_unnormalized_pdf(theta_prev, omega_prev, internal_cell_counts_samp, mu_samp, Sigma_samp)
            ...
            for r_prime in range(r+1, num_groups):
                for c in range(num_candidates-1):
                    for c_prime in range(c+1, num_candidates):
                            n1 = prev_internal_counts_samp[i, r, c] + prev_internal_counts_samp[i, r, c_prime] # row total in row r_prime
                            n2 = prev_internal_counts_samp[i, r_prime, c] + prev_internal_counts_samp[i, r_prime, c_prime]  # row total in row r
                            m1 = prev_internal_counts_samp[i, r, c] + prev_internal_counts_samp[i, r_prime, c] # column total in column c
                            ...
                            prev_internal_counts_samp[i, r_prime, c_prime] = n2 - prev_internal_counts_samp[i, r_prime, c]
...
def get_initial_internal_count_sample(group_counts, vote_counts, precinct_pops):
    ...
    for i in range(num_precincts):
        for r in range(num_groups-1):
            for c in range(num_candidates-1):
                n = np.round(group_counts[i, r] * (vote_counts[i, c] + vote_counts[i, c+1]) / precinct_pops[i]).astype(int)
                #print(n)
            ...
            vote_counts_remaining[i, num_candidates-1] = vote_counts_remaining[i, num_candidates-1] - internal_counts_samp[i, r, num_candidates-1] 
